...
enable_qcawifi() {
	...
	config_get aggr_burst "$device" aggr_burst
	[ -n "$aggr_burst" ] && iwpriv "$phy" aggr_burst $aggr_burst
	...
	for vif in $vifs; do
		...
		case "$mode" in
			sta)
				...
				config_get wlanaddr "$device" wlanaddr
				;;
			adhoc)
		...
		config_get ldpc "$vif" ldpc 0
		iwpriv "$ifname" ldpc "$ldpc"
...
pre_qcawifi() {
	...
	case "${action}" in
		disable)
			config_foreach qwrap_stop wifi-device
...
qwrap_start() {
	local device=$1
	local ifidx=0
	local radioidx=${device#wifi} iso="" wrapd_vma_conf=""
	...
	config_get_bool ap_isolation_enabled $device ap_isolation_enabled 0
	[ $ap_isolation_enabled -ne 0 ] && iso="-I"
	...
	wrapd ${iso} -P $wrapd_pid -D $device -c $wrapd_conf_file -a $wrapd_ifname -p $sta_ifname $wrapd_vma_conf -g $wrapd_ctrl_interface -w $wpa_supplicant_global_ctrl_iface -B
}
...
post_qcawifi() {
	...
	case "${action}" in
		enable)
			...
			# Check for QWRAP enabled radios
			config_foreach qwrap_start wifi-device
		;;
	esac
}
